...
public class BooksController
{
    ...
    public int checkAvailability(String copyID)
    {
        ...
        try
        {
            ...
            // Send the copyID to the server to check the availability of the copyID
            ClientUI.chat.accept(new MessageType("106", copyID));

            // Wait for the server's response

            // check if the copy of the book is available
            if (ChatClient.availability.get(0).equals("1"))
            {
                //available for everyone
                return 0;
                //book has orders so it available for a wait list only
            }
